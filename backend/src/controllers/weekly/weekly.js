...
module.exports.weekly = async (req, res) => {
    ...
    try {
        const classObj = await Class.findOne({ code: classCode }).select('_id, code');
    
        if (!classObj) {
          return res.status(404).json({ error: "Class not found" });
        }
     
        const activeSemester = await Semester.findOne( {status: "active"} ).select('_id weeks');
        ...
        if (!activeClassSemester) {
          ...
          console.log(activeClassSemester.sections[0].weeklyContent);
        res.json({
