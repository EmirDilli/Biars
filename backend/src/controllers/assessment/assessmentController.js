...
module.exports.getAllAssessments = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(
        generateResponse("Assessments retrieved successfully", { assessments })
      );
  } catch (error) {
    console.error("Error retrieving assessments:", error);
    res
      .status(500)
      .json(generateResponse("Server Error", { error: error.toString() }));
  }
};
...
module.exports.getAssessmentWithId = async (req, res) => {
  try {
    ...
    if (!assessmentId) {
      return res
        .status(400)
        .json(generateResponse("No Assessment ID provided", {}));
    }
    ...
    res
      .status(200)
      .json(
        generateResponse("Assessment retrieved successfully", { assessment })
      );
  } catch (error) {
...
module.exports.createAssessmentWithQuestion = async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .send({
        message:
          "Missing questionId, assessmentData, weight, or max in request body.",
      });
  }
  ...
  try {
    ...
    if (!questionExists) {
      return res
        .status(404)
        .send({ message: "Question not found with the provided ID." });
    }
    ...
    res
      .status(500)
      .send({ message: "Failed to create assessment", error: error.message });
  }
};
// **********************************************************************************************************
...
module.exports.removeAssessment = async (req, res) => {
  try {
    ...
    if (!assessmentId) {
      return res
        .status(400)
        .json(generateResponse("Missing Assessment ID", {}));
    }
    ...
    return res
      .status(200)
      .json(
        generateResponse("Assessment Removed Successfully", {
          id: assessmentId,
        })
      );
  } catch (error) {
...
module.exports.viewOldAssessments = async (req, res) => {
  try {
    ...
    if (!semester) {
      return res
        .status(400)
        .json(generateResponse("Semester parameter is required", {}));
    }
    ...
    if (assessments.length === 0) {
      return res
        ...
        .json(
          generateResponse(
            "No assessments found for the specified semester",
            {}
          )
        );
    }
...
module.exports.getQuestionHistory = async (req, res) => {
  ...
  try {
    ...
    if (history.length === 0) {
      return res
        .status(404)
        .json({ message: "No assessments found with the given question" });
    }
...
module.exports.createAssessment = async (req, res) => {
  try {
    ...
    const totalQuestions = Object.values(topics).reduce(
      (acc, num) => acc + num,
      0
    );
...
module.exports.addQuestionToAssessment = async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .send({
        message:
          "Assessment ID, Question ID, weight, and max score are required.",
      });
  }
  ...
  try {
    ...
    const updatedAssessment = await Assessment.findByIdAndUpdate(
      ...
      {
        $addToSet: {
          questions: { question: questionId, weight: weight, max: max },
        },
    ...
    return res
      .status(500)
      .send({ message: "Failed to add question to assessment" });
  }
};
//1.8 TODO
...
module.exports.removeQuestionFromAssessment = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(generateResponse("Question Removed Successfully", { assessment }));
  } catch (error) {
...
module.exports.createAssessmentFromQuestions = async (req, res) => {
  ...
  for (let question of questions) {
    if (!isValidObjectId(question.questionId)) {
      ...
      return res
        .status(400)
        .json({ message: "Invalid questionId in the list" });
    }
  }
  ...
  try {
    ...
    const classSemester = await ClassSemester.findOne({
      class: courseId,
    ...
    console.log(
      await ClassPortfolio.updateOne(
        { classSemester: classSemester._id },
        { $push: { "studentPerformance.$[].grades": {} } }
      )
    );
    console.log(
      await ClassSemester.updateOne(
        ...
        {
          $push: {
            assessments: { assessment: assessment._id, weight: weight },
          },
        }
      )
    );
